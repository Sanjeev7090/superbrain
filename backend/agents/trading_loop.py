...
class TradingLoop:
    ...
    def _run_cycle(self) -> None:
        ...
        try:
            ...
            # Central Brain (HybridSuperBrain) is the SOLE decision maker:
            #   DreamerV3 -> StrategyCollaborator -> MiroFish ->
            #   MetaReasoner -> Quality Filter -> Survival -> Risk Gate
            # ════════════════════════════════════════════════════════════════
            ...
            for ticker in watchlist:
                ...
                # Sync brain daily PnL before deciding
                ...
                last_brain_block = decision

                # Raw DreamerV3 confidence for display in Watchlist panel
                dreamer_conf_display = 0
                try:
                    from rl_agent.dreamer_trainer import get_prediction as _gp
                    _pred = _gp(ticker)
                    dreamer_conf_display = round(float(_pred.get("confidence", 0)), 1)
                except Exception:
                    dreamer_conf_display = round(float(confidence * 0.9), 1)
                ...
                dreamer_dir = 1.0 if signal == "BUY" else (-1.0 if signal == "SELL" else 0.0)

                obs_entry.update({
                    "signal":         signal,
                    "confidence":     round(float(confidence), 1),
                    "brain_action":   signal,
                    "brain_conf":     round(float(confidence), 1),
                    "brain_fear":     round(brain_fear, 3),
                    "dreamer_signal": decision.get("strategy_consensus", signal),
                    "dreamer_conf":   dreamer_conf_display,
                    "dreamer_dir":    dreamer_dir,
                    "brain_reason":   reasoning,
                ...
                if signal == "HOLD" or confidence <= 58:
                    ...
                    logger.info(
                        "[TradingLoop][%s] %s -> %s | conf=%.0f%% quality=%.0f",
                        cycle_id, ticker, signal, confidence, quality,
                    )

                # Update global current_decision
                _upd(
                    current_decision   = {**decision, "ticker": ticker},
                    dreamer_signal     = dreamer_dir,
                    dreamer_confidence = confidence,
                    dreamer_weights    = {},
                    dreamer_wm_loss    = 0.0,
                ...
                try:
                    ...
                    try:
                        ...
                        layer_evolution.evolve_from_live_training(
                            ticker, live_reward, decision, signal, confidence,
                        )
            ...
            # Update shared brain state from last ticker decision
            if last_brain_block:
                _upd(
                    brain_active        = True,
